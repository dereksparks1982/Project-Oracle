...
root_executable="$project_root/Project_Oracle_v0_0_17"

fail() { echo "VALIDATION FAIL: $*" >&2; exit 1; }
blocked() { echo "VALIDATION BLOCKED: $*" >&2; exit 2; }

command -v dotnet >/dev/null 2>&1 || blocked 'Project Oracle v0.0.17 requires the .NET 10 SDK. No C# build or acceptance tests were run.'
...
file "$root_executable" | grep -Fq 'ELF' || fail 'Project_Oracle_v0_0_17 is not an ELF executable.'
...
grep -Fq 'Monad cast Yala into the Void.' src/ProjectOracle.Core/Lore/OracleLore.cs || fail 'Yala Void lore missing.'
...
if grep -RIniE --exclude='PROJECT_ORACLE_CHANGELOG.md' --exclude-dir=bin --exclude-dir=obj \
  'Creator / Omega|Creator/Omega|autonomous in-world Oracle' \
  ...
  fail 'active source/docs still contain superseded primordial or in-world Oracle wording.'
fi
...
for path in \
  ...
  docs/PROJECT_ORACLE_LORE_CANON_v0_0_17.md \
  docs/PROJECT_ORACLE_CANON_v0_0_17.md \
  docs/PROJECT_ORACLE_ARCHITECTURE_v0_0_17.md \
  docs/PROJECT_ORACLE_ROADMAP_v0_0_17.md \
  docs/PROJECT_ORACLE_VALIDATION_v0_0_17.md \
  docs/PROJECT_ORACLE_WORLD_TIME_INTAKE_v0_0_17.md \
  docs/PROJECT_ORACLE_RESUME_HANDSHAKE_v0_0_17.md; do
  [[ -f "$path" ]] || fail "current authority missing: $path"
  grep -Fq 'v0.0.17' "$path" || fail "current version marker missing from $path"
done
...
grep -Fq 'Soar 9.6.5' README.md || fail 'README Soar 9.6.5 statement missing.'
echo 'PHASE PASS: README and authority alignment'

echo 'PHASE START: fresh-save and pre-Time architecture gates'
grep -Fq 'save_v2.json' src/ProjectOracle.Core/Persistence/OracleSaveStore.cs || fail 'v0.0.17 fresh save_v2 path missing.'
if grep -Fq '"0.0.16"' src/ProjectOracle.Core/Persistence/OracleSaveStore.cs; then fail 'v0.0.16 must not be accepted by the v0.0.17 save loader.'; fi
grep -Fq 'v0.0.16 save snapshots are rejected instead of migrated' tests/ProjectOracle.AcceptanceTests/Program.cs || fail 'v0.0.16 save rejection regression missing.'
...
echo 'PHASE PASS: fresh-save and pre-Time architecture gates'
...
PROJECT_ORACLE_WINDOW_DRY_RUN=1 ./scripts/run-window.sh | grep -Fq 'Project Oracle v0.0.17 - Yala Soar Console' || fail 'window dry-run identity wrong.'
grep -Fq 'PROJECT_ORACLE_EXEC_PLACEHOLDER' desktop/project-oracle.desktop || fail 'desktop executable placeholder missing.'
grep -Fq 'Project_Oracle_v0_0_17' scripts/install-desktop-launcher.sh || fail 'desktop installer does not target root v0.0.17 executable.'
...
echo 'PHASE START: published executable Soar smoke'
set +e
...
grep -Fq 'Project Oracle v0.0.17' <<<"$smoke_output" || fail 'published smoke version missing.'
grep -Fq 'SOAR SMOKE PASS:' <<<"$smoke_output" || fail 'published executable did not prove a real Soar Yala decision.'
echo 'PHASE PASS: published executable Soar smoke'
