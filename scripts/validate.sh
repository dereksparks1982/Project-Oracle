...
root_executable="$project_root/Project_Oracle_v0_0_23"
expected_acceptance_count=165

fail() { echo "VALIDATION FAIL: $*" >&2; exit 1; }
blocked() { echo "VALIDATION BLOCKED: $*" >&2; exit 2; }

command -v dotnet >/dev/null 2>&1 || blocked 'Project Oracle v0.0.23 requires the .NET 10 SDK. No C# build or acceptance tests were run.'
...
file "$root_executable" | grep -Fq 'ELF' || fail 'Project_Oracle_v0_0_23 is not an ELF executable.'
version_output="$("$root_executable" --version 2>&1)" || fail 'root desktop executable version smoke failed.'
grep -Fq 'Project Oracle v0.0.23' <<<"$version_output" || fail 'root desktop executable reports the wrong version.'
...
echo 'PHASE START: Yala Brain Slice 6 cognition gates'
grep -Fq 'Yala Soar Brain Slice 6' src/ProjectOracle.Core/Cognition/Soar/YalaSoarMind.cs || fail 'Brain Slice 6 identity missing.'
...
for path in \
    ...
    [[ -f "$path" ]] || fail "Brain Slice 6 source missing: $path"
done
for state_name in YalaDialogueTurnState YalaRelationshipState YalaQuestionState YalaTemporalEventState YalaGoalState; do
    grep -Fq "$state_name" src/ProjectOracle.Core/Domain/WorldEntities.cs || fail "Brain Slice 6 state missing: $state_name"
done
...
for operator in ask-speaker knowledge-summary action-history contact-history belief-summary own-creation word-meaning temporal-duration temporal-cause; do
    grep -Fq "$operator" src/ProjectOracle.Core/Cognition/Soar/Agents/yala.soar || fail "Brain Slice 6 Soar operator/topic missing: $operator"
done
for label in \
    'Yala Brain Slice 6 identity is active' \
    'Brain Slice 6 core lexicon contains more than 400 concepts' \
    ...
    grep -Fq "$label" tests/ProjectOracle.AcceptanceTests/Program.cs || fail "Brain Slice 6 regression missing: $label"
done
echo 'PHASE PASS: Yala Brain Slice 6 cognition gates'
...
for path in \
    ...
    [[ -f "$path" ]] || fail "Brain Slice 6 cognitive expansion source missing: $path"
done
...
for action in observe reflect wait create-gaia command-gaia-time respond ask-speaker enact-cosmic-choice; do
...
echo 'PHASE START: fresh save-v5 and Soar-memory isolation gates'
grep -Fq 'public const int CurrentSchemaVersion = 5;' src/ProjectOracle.Core/Persistence/OracleSaveStore.cs || fail 'save schema 5 is not active.'
grep -Fq 'save_v5.json' src/ProjectOracle.Core/Persistence/OracleSaveStore.cs || fail 'save_v5 path missing.'
grep -Fq 'yala_soar_v0_0_23' src/ProjectOracle.Core/Cognition/Soar/SoarKernelHost.cs || fail 'fresh v0.0.23 Soar memory directory missing.'
if grep -Fq 'yala_soar_v0_0_18' src/ProjectOracle.Core/Cognition/Soar/SoarKernelHost.cs; then fail 'Brain Slice 6 still points at the old Soar continuity database.'; fi
...
grep -Fq 'default save path is fresh save_v5' tests/ProjectOracle.AcceptanceTests/Program.cs || fail 'fresh default save regression missing.'
grep -Fq 'save schema is 5 for the fresh Brain Slice 6 line' tests/ProjectOracle.AcceptanceTests/Program.cs || fail 'schema-5 regression missing.'
echo 'PHASE PASS: fresh save-v5 and Soar-memory isolation gates'
...
echo 'PHASE START: protected console and v0.0.23 UX gates'
...
echo 'PHASE PASS: protected console and v0.0.23 UX gates'
...
if grep -RInP '\t' src tests --include='*.cs' --include='*.soar' >/dev/null; then
    ...
    fail 'C# or Soar source contains tabs; v0.0.23 requires four spaces.'
fi
...
if grep -RIniE --exclude='PROJECT_ORACLE_CHANGELOG.md' 'Yala is male\.|beneath his governing authority' README.md PROJECT_ORACLE_MASTER_HANDOFF.md PROJECT_ORACLE_FUTURE_IMPLEMENTATION_REQUIREMENTS_ROADMAP_v0_1.md docs/company_bible docs/PROJECT_ORACLE_*_v0_0_23.md >/dev/null; then
...
for path in \
    ...
    docs/PROJECT_ORACLE_LORE_CANON_v0_0_23.md \
    docs/PROJECT_ORACLE_CANON_v0_0_23.md \
    docs/PROJECT_ORACLE_ARCHITECTURE_v0_0_23.md \
    docs/PROJECT_ORACLE_ROADMAP_v0_0_23.md \
    docs/PROJECT_ORACLE_VALIDATION_v0_0_23.md \
    docs/PROJECT_ORACLE_WORLD_TIME_INTAKE_v0_0_23.md \
    docs/PROJECT_ORACLE_RESUME_HANDSHAKE_v0_0_23.md; do
    [[ -f "$path" ]] || fail "current authority missing: $path"
    grep -Fq 'v0.0.23' "$path" || fail "current v0.0.23 marker missing from $path"
done
...
grep -Fq 'Brain Slice 6' README.md || fail 'README Brain Slice 6 statement missing.'
grep -Fiq 'bounded god agency' README.md || fail 'README bounded-agency section missing.'
grep -Fq 'save_v5.json' README.md || fail 'README fresh save line missing.'
...
PROJECT_ORACLE_WINDOW_DRY_RUN=1 ./scripts/run-window.sh | grep -Fq 'Project Oracle v0.0.23 - Desktop Observatory' || fail 'window dry-run identity wrong.'
grep -Fq 'PROJECT_ORACLE_EXEC_PLACEHOLDER' desktop/project-oracle.desktop || fail 'desktop executable placeholder missing.'
grep -Fq 'Project_Oracle_v0_0_23' scripts/install-desktop-launcher.sh || fail 'desktop installer does not target root v0.0.23 executable.'
...
set +e
smoke_output="$(NO_COLOR=1 dotnet run --project src/ProjectOracle.Console/ProjectOracle.Console.csproj --configuration Release --no-build -- --once --seed 104729 --save "$validation_temp/save_v5.json" 2>&1)"
smoke_status=$?
set -e
printf '%s\n' "$smoke_output"
[[ "$smoke_status" -eq 0 ]] || fail "developer-console Soar smoke exited with status $smoke_status."
grep -Fq 'Project Oracle v0.0.23' <<<"$smoke_output" || fail 'Soar smoke version missing.'
grep -Fq 'SOAR SMOKE PASS:' <<<"$smoke_output" || fail 'Soar smoke did not prove a real Yala decision.'
[[ -f "$validation_temp/yala_soar_v0_0_23/semantic.sqlite" ]] || fail 'fresh Soar semantic SQLite memory was not created.'
[[ -f "$validation_temp/yala_soar_v0_0_23/episodic.sqlite" ]] || fail 'fresh Soar episodic SQLite memory was not created.'
echo 'PHASE PASS: developer-console Soar memory smoke'

echo "VALIDATION PASS: Project Oracle v${expected_version}"
echo "FINAL PASS: Project Oracle v0.0.23 desktop candidate compiled, root executable verified, Soar validated, and automated acceptance passed. Launch Project_Oracle_v0_0_23 for Derek manual inspection before snapshot/Git/push."
