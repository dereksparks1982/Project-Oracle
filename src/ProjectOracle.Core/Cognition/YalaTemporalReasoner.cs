...
public static class YalaTemporalReasoner
{
    ...
    {
        "before-time" => $"{item.Summary} It happened before Time existed, so there was no in-world date.",
        ...
        long currentWorldMilliseconds)
    {
        ...
        if (item.TemporalState == "before-time") return $"{item.Summary} It happened before Time existed, so no in-world duration can measure how long ago it was.";
